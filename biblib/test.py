...
class BibParserTest(unittest.TestCase):
    ...
    def test_basic(self):
        self.__test_parse(
            ...
            [
                ent("misc", "x", od("title", "title", "author", "author")),
    ...
    def test_balanced(self):
        self.__test_parse(
            ...
            [ent("misc", "x", od("title", "a{b}c", "author", "x{y}z"))],
        )

    def test_whitespace(self):
        self.__test_parse(
            " @ misc { x , title = {a} , }", [ent("misc", "x", od("title", "a"))]
        )
    ...
    def test_funny_keys(self):
        self.__test_parse(
            '@misc{@"#%\'()=, title="a"}', [ent("misc", "@\"#%'()=", od("title", "a"))]
        )
        for string in ["@misc{}", "@misc{,}", "@misc{\n}"]:
            self.__test_parse(string, [ent("misc", "", od())])
        self.__test_parse('@misc{,title="a"}', [ent("misc", "", od("title", "a"))])
        self.__test_parse(
            '@misc(k)ey, title="a")\n@misc{k{ey, title="a"}',
            [
  ...
  year         = 2013,
    ...
    def test_date_key(self):
        def test(year, month, expect):
            ...
            entry = Entry(fields, field_pos={"year": Pos.unknown, "month": Pos.unknown})
            got = entry.date_key()
...
class CrossRefTest(unittest.TestCase):
            ...
            [
                (
                    ...
                    ent(
                        "misc",
                        "ent1",
                        [("title", "Title 1"), ("booktitle", "Book title 2")],
                    ),
                ),
                (
                    "ent2",
                    ent(
                        "misc",
                        "ent2",
                        [("title", "Title 2"), ("booktitle", "Book title 2")],
                    ),
                ),
            ],
            list(db.items()),
    ...
    def test_min_crossrefs(self):
        ...
        self.assertEqual(
            [
                (
                    "ent1",
                    ent("misc", "ent1", [("title", "Title 1"), ("crossref", "ent2")]),
                ),
                (
                    "ent2",
                    ent(
    ...
    def test_self_crossref(self):
        ...
        log = io.StringIO()
        self.parser.parse(
            "@misc{ent3, title={Title 3}, crossref={ent3}}", name="<ent3>", log_fp=log
        )
...
class NameParserTest(unittest.TestCase):
    ...
    def test_and(self):
        p = parse_names
        self.assertEqual(
            p("A B and C D"), [Name("A", "", "B", ""), Name("C", "", "D", "")]
        )
        self.assertEqual(
            p("A B AND C D"), [Name("A", "", "B", ""), Name("C", "", "D", "")]
        )
        self.assertEqual(
            p("A B and and C D"),
            [Name("A", "", "B", ""), Name("", "", "", ""), Name("C", "", "D", "")],
        )
        self.assertEqual(
            p("A B and and"), [Name("A", "", "B", ""), Name("", "", "and", "")]
        )
        self.assertEqual(p("A B { and } C D"), [Name("A B { and } C", "", "D", "")])
        self.assertEqual(p("A B {\\ and } C D"), [Name("A B", "{\\ and }", "C D", "")])
    ...
    def __test_names(self, *tests):
        for test in tests:
            ...
            self.assertEqual(names, [Name(*test[1:])], "parsing {!r}".format(test[0]))
...
class NamePrettyTest(unittest.TestCase):
    ...
    def test_basic(self):
        self.__test("{first}{von}{jr}{last}", lambda n: n.first + n.von + n.jr + n.last)
        self.__test(
            "{first} {von} {last} {jr}",
    ...
    def test_before_and_after(self):
        ...
        self.__test(
            ...
            lambda n: "a"
            + self.__clean(" ".join([n.first, n.von, n.last, n.jr]))
            + "b",
        )
...
class CaseTest(unittest.TestCase):
    def __test(self, *tests):
        for string, want in tests:
            self.assertEqual(
                title_case(string), want, "title-casing {!r}".format(string)
            )
