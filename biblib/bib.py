...
class Parser:
    ...
    def __init__(self, *, month_style="full"):
        ...
        if month_style == "full":
            ...
            raise ValueError(f"Unknown month style {month_style}")
    ...
    def parse(self, str_or_fp_or_iter, name=None, *, log_fp=None):
        ...
        elif isinstance(str_or_fp_or_iter, collections.Iterable) and not hasattr(
            str_or_fp_or_iter, "read"
        ):
    ...
    def _scan_command_or_entry(self):
        ...
        if typ == "string":
            ...
            if name in self.__macros:
                self._warn(f"macro `{name}' redefined")
    ...
    def _scan_field_piece(self):
        ...
        if piece is not None:
            if piece.lower() not in self.__macros:
                self._warn(f"unknown macro `{piece}'", opos)
                return ""
...
class FieldError(KeyError):
    ...
    def __str__(self):
        return f"{self.__entry}: missing field `{self.args[0]}'"
...
class Entry(collections.OrderedDict):
    ...
    def date_key(self):
        ...
        if year is not None:
            if not year.isdigit():
                self.field_pos["year"].raise_error(f"invalid year `{year}'")
            key += (int(year),)
...
def resolve_crossrefs(db, min_crossrefs=None):
    ...
    if min_crossrefs is not None:
        counts = collections.Counter(
            entry["crossref"].lower() for entry in db.values() if "crossref" in entry
        )
    else:
        counts = None
            ...
            with recoverer:
                ...
                if crossref_idx is None:
                    entry.field_pos["crossref"].raise_error(
                        f"unknown crossref `{crossref}'"
                    )
                elif crossref_idx < entry_idx:
                    entry.field_pos["crossref"].raise_error(
                        f"crossref `{crossref}' must come after entry"
                    )
